...
var findUser = function(db, callback) {
    var cursor =db.collection('patientlist').find( );
...
var findUserwithName = function(db,callback) {
    var cursor = db.collection('patientlist').find({"fname":"Onica"});
    cursor.each(function(err,doc) {
        assert.equal(err,null);
        if(doc != null)
        {
...
var findUserwithUniversity = function(db, callback) {
    var cursor = db.collection('patientlist').find({"education.university":"UMKC"});
    cursor.each(function(err,doc){
       assert.equal(err,null);
       if(doc != null)
       {
